...
require_contains "index.html" "Screen 09 boundary"
...
echo "== Guardrails =="
if grep -Eiq 'Stripe|card number|paymentIntent|type="password"|passkey|fetch\(|XMLHttpRequest|localStorage|sessionStorage|dashboard|followers|trending' index.html script.js; then
...
if grep -Eiq 'id="code-input"|verification code input|otp' index.html; then
  echo "FAIL: Screen 09 verification code UI present"
  fail=1
else
  echo "OK: no verification-code UI"
fi
