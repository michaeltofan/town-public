(() => {
  ...
  const imageCompression = window.TownImageCompression;
  if (!imageCompression) {
    throw new Error("TownImageCompression must load before script.js");
  }
  const { loadImageForCompression, canvasToBlobAsync, withExtension } =
    imageCompression;
  ...
  const apiClient = window.TownApiClient;
  if (!apiClient) {
    throw new Error("TownApiClient must load before script.js");
  }
  const {
    requestJson,
    fetchJson,
  ...
  const geoBoundary = window.TownGeoBoundary;
  if (!geoBoundary) {
    throw new Error("TownGeoBoundary must load before script.js");
  }
  const { geojsonContainsPoint } = geoBoundary;
