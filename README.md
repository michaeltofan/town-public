...
Operators sign in on `/platform/` with passkey. Access still requires an account
granted via `account:mark-platform-operator` on town-api (or later by
`role_admin` in the Operators panel). Talks to `/v1/platform/*`.
